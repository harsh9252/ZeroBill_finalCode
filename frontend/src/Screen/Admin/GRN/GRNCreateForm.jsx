...
/* ---------- Main GRNCreateForm (Goods Receipt Note Form) ---------- */
export default function GRNCreateForm({
  onSave,
  onBack,
  initialData = {},
  products: initialProducts = [],
  formTitle = "Create GRN",
  saveLabel = "Save",
  cancelLabel = "Cancel",
  ...
  const [grnNoError, setGRNNoError] = useState("");

  // GRN Number Variables
  const [grnNo, setGrnNo] = useState(initialData?.grn_number || "");

  // Sync state when initialData arrives or changes (important for edit mode)
  useEffect(() => {
    if (initialData) {
      if (initialData.grn_number) setGrnNo(initialData.grn_number);
      ...
      if (initialData.date || initialData.grn_date) setInvoiceDate((initialData.date || initialData.grn_date).split('T')[0]);
  ...
  // Automactic generate grn number
  useEffect(() => {
    const fetchGRNNumber = async () => {
      try {
        ...
        const res = await grnAPI.getNextNumber(businessId);
        console.log("GRN API Response:", res);
        ...
        const financialYear = `${year}-${(year + 1)
          .toString()
          .slice(-2)}`;
        setGrnNo(`GRN-${financialYear}-0001`);
      } catch (err) {
        console.error(err);

        // fallback on error
        const year = new Date().getFullYear();
        setGrnNo(`GRN-${year}-0001`);
      }
    };

    if (!initialData?.grn_number && !initialData?.dbId && !initialData?.id) {
      fetchGRNNumber();
    }
  }, [initialData?.grn_number, initialData?.dbId, initialData?.id]);
  ...
  console.log('advice no', AdviceNoteNumber)
  // GRN-specific fields
  ...
  const [termsHeading, setTermsHeading] = useState(
    initialData.meta?.termsHeading || "Terms & Conditions",
  );

  // Determine if this is an editing form
  const isEditing = !!initialData.grn_number;
  ...
  const [lines, setLines] = useState(() => {
    // Try to get lines from different possible locations (top-level first, then meta)
    const lineItems =
      initialData.products ||
      initialData.meta?.lines ||
      initialData.grn_data?.lines ||
  ...
  useEffect(() => {
    ...
    if (newGrnNumber) setGrnNo(newGrnNumber);
  }, [
    initialData.poAgreementNumber,
    initialData.po_agreement_number,
    initialData.meta?.poAgreementNumber,
  ...
  // Fetch next GRN number when form loads for new documents
  useEffect(() => {
    const fetchNextDocumentNumber = async () => {
      ...
      try {
        ...
        if (!selectedBusinessId) {
          ...
          const defaultNumber = `GRN-${financialYear}-0001`;
          setGrnNo(defaultNumber);
          return;
        }

        const res = await grnAPI.getNextNumber(selectedBusinessId);
        if (res?.success) {
          const number = res?.data?.grn_number || res?.data?.number || res?.data;
          if (number) {
            setGrnNo(number);
          }
        } else {
  ...
  async function handleSubmit(e) {
    ...
    const invoiceData = {
      ...
      party_id: resolvedPartyId,
      bill_to_address_id:
        selectedBillingAddr?.id || initialData.bill_to_address_id || null,
    ...
    // Add grnId for edit mode
    if (isEditing && initialData.dbId) {
      invoiceData.grnId = initialData.dbId;
    }
    ...
    if (onSave) {
      try {
        ...
        if (errorData?.field === "grn_number") {
          setGRNNoError(errorData.message || "GRN number already exists");
          // Show error toast as well for visibility
          showErrorToast(errorData.message || "GRN number already exists");
        } else {
                    ...
                    <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div>
                        ...
                        <input
                          ...
                          className={`w-full px-4 py-2 border-2 rounded-lg text-sm focus:ring-2 focus:outline-none transition-colors ${grnNoError ? "border-red-500 focus:border-red-500 focus:ring-red-500/20" : "border-gray-200 focus:border-[#1fbe5a] focus:ring-[#1fbe5a]/20"}`}
                          disabled={submitting}
                        />
                        {grnNoError && (
