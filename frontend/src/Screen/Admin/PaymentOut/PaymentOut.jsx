...
const { partyAPI, paymentOutAPI, salesInvoiceAPI, bankDetailsAPI } = api;
...
function RecordPaymentOutForm({ onBack, editData, onSave, currency = 'INR' }) {
  ...
  useEffect(() => {
    if (prevCurrencyRef.current !== currency) {
      setFormData(prev => {
        const amountInINR = convertToINR(parseFloat(prev.amountPaid) || 0, prevCurrencyRef.current);
        const discountInINR = convertToINR(parseFloat(prev.paymentOutDiscount) || 0, prevCurrencyRef.current);

        return {
          ...prev,
          amountPaid: amountInINR > 0 ? convertAmount(amountInINR, currency).toFixed(2) : '',
          paymentOutDiscount: discountInINR > 0 ? convertAmount(discountInINR, currency).toFixed(2) : ''
        };
      });
  ...
  useEffect(() => {
    if (editData) {
      ...
      setFormData(prev => ({
        ...
        amountPaid: editData.amount ? convertAmount(editData.amount, currency).toFixed(2) : '',
        paymentOutDiscount: editData.discount ? convertAmount(editData.discount, currency).toFixed(2) : '',
        paymentDate: editData.date || new Date().toISOString().split('T')[0],
        paymentMode: editData.paymentMode || 'Cash',
        paymentPaidFrom: '',
        paymentOutNumber: editData.id || '1',
  ...
  // Auto-allocate amountPaid to linked invoices
  useEffect(() => {
  ...
  }, [formData.amountPaid, formData.isInvoiceLinked, formData.linkedInvoices.length]);
  ...
  useEffect(() => {
    const fetchPartyData = async () => {
      ...
      try {
        // 1. Fetch sales invoices for this party (Linked to Tax Invoices for Payment Out)
        ...
        // 2. Fetch ALL historical payments OUT for this party
        const paymentsResponse = await api.paymentOutAPI.getAll(selectedBusinessId);
  ...
  useEffect(() => {
    ...
    const fetchPaymentData = async () => {
      ...
      try {
        ...
        const response = await api.paymentOutAPI.getById(editData.dbId, selectedBusinessId);
        ...
        if (response.success && response.data) {
          ...
          setFormData(prev => ({
            ...
            amountPaid: (payment.amount_received || editData.amount) ? convertAmount(payment.amount_received || editData.amount, currency).toFixed(2) : '',
            paymentOutDiscount: (payment.payment_discount || editData.discount) ? convertAmount(payment.payment_discount || editData.discount, currency).toFixed(2) : '',
            paymentDate: payment.payment_date || editData.date || new Date().toISOString().split('T')[0],
            paymentMode: payment.payment_mode || 'Cash',
            paymentPaidFrom: payment.payment_received_in || '',
            paymentOutNumber: payment.payment_number || editData.id || '1',
            notes: payment.notes || '',
  ...
  useEffect(() => {
    ...
    const generatePaymentNumber = async () => {
      try {
        const response = await paymentOutAPI.getNextNumber(selectedBusinessId);
        if (response.success) {
          setFormData(prev => ({ ...prev, paymentOutNumber: response.data }));
        }
      } catch (error) {
  ...
  const handleInputChange = (field, value) => {
    ...
    if (field === 'paymentMode' && value === 'Cash') {
      setFormData(prev => ({ ...prev, paymentPaidFrom: '' }));
    }

    if (field === 'paymentPaidFrom' && value === 'add_new') {
  ...
  const handleBankAccountSubmit = (accountData) => {
    ...
    setFormData(prev => ({ ...prev, paymentPaidFrom: newAccount.id.toString() }));
    setShowBankModal(false);
    showSuccessToast('Bank added');
  };

  const isFormComplete = formData.partyName && formData.amountPaid && parseFloat(formData.amountPaid) > 0;
  ...
  const handleSaveClick = async () => {
    ...
    try {
      ...
      const paymentData = {
        payment_number: formData.paymentOutNumber,
        party_id: formData.partyId,
        party_name: formData.partyName,
        amount_received: convertToINR(parseFloat(formData.amountPaid) || 0, currency),
        payment_discount: formData.paymentOutDiscount ? convertToINR(parseFloat(formData.paymentOutDiscount), currency) : 0,
        payment_date: formData.paymentDate,
        payment_mode: formData.paymentMode,
        payment_received_in: formData.paymentPaidFrom || null,
      ...
      if (isEditMode && editData) {
        ...
        const response = await paymentOutAPI.update(editData.dbId, selectedBusinessId, paymentData);
        setSubmitting(false);
        ...
        // Create new payment
        const response = await paymentOutAPI.create(selectedBusinessId, paymentData);
        setSubmitting(false);
  ...
  const showPaymentPaidFrom = formData.paymentMode !== 'Cash';
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-yellow-900">
              <span>{isEditMode ? <span>Edit Payment <span>{formData.paymentOutNumber}</span></span> : <span>Record Payment Out</span>}</span>
            </h1>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 mb-3">
            ...
            <div className="border border-gray-200 rounded p-3 md:p-4 flex flex-col">
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                <div>
                  <CustomNumberInput
                    label={<>Amount Paid <span className="text-red-500">*</span></>}
                  ...
                  {fieldErrors.amountPaid && (
                    <p className="mt-1 text-xs text-red-500 font-medium">{fieldErrors.amountPaid}</p>
                  )}
                </div>
                <CustomNumberInput
                  label="Payment Out Discount"
                  value={formData.paymentOutDiscount}
                  onChange={(val) => handleInputChange('paymentOutDiscount', val)}
                  placeholder="0"
                  currencySymbol={currencySymbol}
                />
              </div>
            ...
            <div className="border border-gray-200 rounded p-3 md:p-4 flex flex-col">
              <div className="grid grid-cols-3 gap-2 md:gap-3 mb-3">
                ...
                <div>
                  <label className="block text-xs font-medium text-[#8B4513] mb-1.5">Payment Out #</label>
                  <input type="text" value={formData.paymentOutNumber} onChange={(e) => {
                    handleInputChange('paymentOutNumber', e.target.value);
                  ...
                  <table className="w-full text-xs text-left border-collapse">
                    <thead>
                      <tr className="bg-gray-50 text-gray-700 font-bold border-b text-[11px] uppercase tracking-wider">
                        ...
                        <th className="px-4 py-3 text-right text-[#129046]">Total</th>
                      </tr>
                    </thead>
                    <tbody>
                      {displayTransactions.map((transaction, idx) => (
                        <tr key={idx} className="border-b last:border-0 hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-4 py-3">
                            <span className="font-bold text-gray-800">{transaction.number}</span>
                          </td>
                          <td className="px-4 py-3">
                          ...
                          <td className="px-4 py-3 text-right text-gray-700 font-medium">
                            {formatCurrency(transaction.displayAmount, currency)}
                          </td>
                          <td className="px-4 py-3 text-right text-rose-500 font-medium">
                            {transaction.discount > 0 ? `-${formatCurrency(transaction.discount, currency)}` : '—'}
                          </td>
...
export default function Payment({ currency }) {
  ...
  useEffect(() => {
    const fetchPaymentOuts = async () => {
      ...
      setLoading(true);
      try {
        const response = await paymentOutAPI.getAll(selectedBusinessId);
        if (response.success && response.data) {
        ...
        showErrorToast('Failed to fetch payment outs');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      await paymentOutAPI.delete(itemToDelete.dbId, selectedBusinessId);
  ...
  if (currentView === 'create') {
    return <RecordPaymentOutForm onBack={handleBackToList} onSave={handleFormSave} currency={currency} />;
  }

  if (currentView === 'edit' && editingRow) {
    return <RecordPaymentOutForm onBack={handleBackToList} editData={editingRow} onSave={handleEditSave} currency={currency} />;
  }
  ...
  const columns = [
    ...
    },
    {
      ...
      width: "100px",
      align: "right",
      render: (r) => <span className="font-medium text-gray-900">{formatCurrency(r.amount, currency)}</span>,
    },
    {
      key: "discount",
      title: "Discount",
      ...
      width: "90px",
      align: "right",
      render: (r) => <span className="text-gray-700">{formatCurrency(r.discount, currency)}</span>,
    },
    {
      key: "paymentMode",
      title: "Mode",
      ...
      {currentView === "list" && rows.length === 0 && (
        <div>
          <GeneralEmptyState
            ...
            description="You haven't recorded any payments out yet. Start by recording your first payment out to track your outgoing transactions."
            buttonText="Record First Payment Out"
